  ...
  const qrUrl =
    `https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=${encodeURIComponent(generatedUpiLink)}`;
  ...
  // open UPI apps
  window.open(generatedUpiLink, "_self");
  ...
  return;
        ...
        const message =
          ...
          `Payment Method: ${customer.paymentMethod === "UPI" ? "UPI" : "Cash on Delivery"}%0A` +
          `Payment Status: ${customer.paymentMethod === "UPI" ? "Payment pending verification" : "Collect cash on delivery"}%0A%0A` +
          `Items:%0A${itemsText}%0A%0A` +
          `Total: ₹${total}`;

          // Clear form and cart after COD order
  setCustomer({
    name: "",
    phone: "",
    address: "",
            ...
            <div className="md:col-span-3 space-y-5 animate-fade-in-up">
              ...
              <div className="rounded-2xl bg-bg-card border border-border-subtle p-5">
                ...
                <button
                  ...
                  disabled={locating}
                  className="w-full flex items-center justify-center gap-2 rounded-xl bg-green-100/10 p-3.5 text-sm font-medium text-green-300 transition-all duration-300 hover:bg-green-200/20 hover:text-green-200 active:scale-[0.98] disabled:opacity-50 cursor-pointer"              >
                  <svg
  ...
  className="inline-flex items-center justify-center rounded-full bg-green-600 px-4 py-2 text-sm font-semibold text-white transition-all duration-200 hover:bg-green-500"
>
  I Have Paid
</button>       </div>
                  </div>
